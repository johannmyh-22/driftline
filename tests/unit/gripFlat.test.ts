...
describe('平地基准:油门甩尾', () => {
  ...
  it('中低速满油门满舵能持续甩尾 —— 稳态侧滑角不是入弯瞬态', () => {
    /*
     * 下界 6° 卡的是「持续」这件事:最早那一版 90 km/h 的稳态侧滑角只有 2.0°
     * (入弯瞬间能到 4~7°,之后就回去了),人类三轮反馈的正是这个。
     *
     * 上界 30° 是**发散守卫**,不是手感线:超过这个数就不是甩尾而是打转了,
     * 实测 `TIRE.overdriveSlipMax` 调到 7.0 时 60 km/h 会到 32.6°、且松手收不
     * 干净。手感线由人类试玩定,别拿这两个数当调参目标。
     *
     * **测速点从 [60, 90] 改成 [50, 60, 70],因为人类明确要求高速要收敛** ——
     * 见下面那条新测试。90 km/h 现在是 5.8°,那是**照他的反馈调的**,不是回归。
     * 甩尾该在的区间是中低速,这三个点覆盖它。
     */
    for (const kmh of [50, 60, 70]) {
  ...
  it('高速不该这么容易滑 —— 100 km/h 的稳态侧滑角必须明显小于中低速', () => {
    /*
     * 人类连着两轮反馈「100 左右有点容易打滑」「85 左右也有点滑」。
     *
     * 根因是 `TIRE.overdriveSlipGain` 太陡(15),滑移放大量瞬间顶到
     * `overdriveSlipMax` 的钳位,于是全速域后轮滑移率恒为 0.288 —— 同样的滑移率
     * 在越高车速对应越大的滑移速度,侧滑角就随车速一路涨到 12.6° 才掉下来。
     * gain 调到 6 之后曲线在 60 km/h 见顶、往上单调下降。
     *
     * 上界 8° 的来历:现在实测 100 km/h 是 4.94°,而中低速是 8~9.7°。取 8 是
     * 「高速必须比中低速明显更服帖」的判据,同时留够余量不误报。
     * 把 gain 调回 15 时这里是 12.5°,会红 —— 那正是它要抓的回归。
     */
    const { steady } = driftAt(100);
    expect(steady).toBeLessThan(8);
  });
