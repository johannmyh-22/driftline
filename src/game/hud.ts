  ...
  update(
    ...
    if (race !== null) {
      ...
      if (race.delta !== null && race.deltaTimer > 0) {
        ...
        const deltaClass = isEqual
          ? 'hud-delta-equal'
          : isAhead
            ? 'hud-delta-ahead'
            : 'hud-delta-behind';

        if (deltaStr !== this.lastShownDeltaText || deltaClass !== this.lastShownDeltaClass) {
          this.lastShownDeltaText = deltaStr;
          this.lastShownDeltaClass = deltaClass;
