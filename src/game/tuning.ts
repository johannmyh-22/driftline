...
/**
 * 轮胎模型(Magic Formula + 载荷敏感 + 摩擦圆)。见 src/game/tire.ts。
 *
 * 一旦这四个轮子推不动车,整个「过弯要减速」的物理前提就没了 —— 这里每一个
 * 数字都决定极限在哪里,改动务必连同「为什么」一起写。
 */
export const TIRE = {
  /** 整备质量(kg)。12 太小、1400+ 太船,1200 是家用车常见区间。 */
  mass: 1200,
  /**
   * 单轮额定载荷(牛) = mass·g/4。
   *
   ...
   */
  staticLoadPerWheel: (1200 * 9.81) / 4,
